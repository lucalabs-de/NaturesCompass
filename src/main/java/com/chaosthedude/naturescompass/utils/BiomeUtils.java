...
public class BiomeUtils {
    ...
    // TODO it's very questionable whether we should start with ids instead of the whole biome object
    public static List<Identifier> getAllowedBiomeIDs(World world) {
        ...
        for (Map.Entry<RegistryKey<Biome>, Biome> entry : biomeRegistry.getEntrySet()) {
            ...
            if (biome != null) {
                ...
                RegistryEntry<Biome> biomeEntry = biomeRegistry.getEntry(entry.getKey()).orElseThrow();
                if (biomeID != null && !biomeIDIsBlacklisted(world, biomeID) && biomeEntry.isIn(BiomeTags.IS_OVERWORLD)) {
   ...
   public static List<Biome> getAllowedBiomes(World world) {
       List<Identifier> biomeIds = getAllowedBiomeIDs(world);
       List<Biome> biomes = new ArrayList<>();
       for (Identifier id : biomeIds) {
           getBiomeForIdentifier(world, id).map(biomes::add);
       }
       return biomes;
   }
    ...
    public static String getBiomeTags(World world, Biome biome) {
        ...
        if (biomeRegistry.getKey(biome).isPresent() && biomeRegistry.getEntry(biomeRegistry.getKey(biome).get()).isPresent()) {
            ...
            List<TagKey<Biome>> categoryTags = biomeEntry.streamTags().filter(tag -> tag.id().getPath().startsWith("is_")).toList();
